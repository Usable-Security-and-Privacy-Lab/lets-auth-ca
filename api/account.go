...
func CreateBegin(w http.ResponseWriter, r *http.Request) {
	...
	// Currently this is not the actual user data generated by webauthn and is just dummy data so that the database is hit.
	// The next step is to modify our GORM user model to comply with duo's webauthn interface so that we only have to one set of those models.
	db, err := database.GetDB()
	if err != nil {
		jsonResponse(w, err.Error(), http.StatusInternalServerError)
	}
	gormUser := models.User{Username: username}
	err = db.AddUser(gormUser)
	if err != nil {
		jsonResponse(w, err.Error(), http.StatusInternalServerError)
	}
...
func CreateFinish(w http.ResponseWriter, r *http.Request) {
	...
	// Begin Work In Progress:
	// Adding the credential to user in database.
	// Currently this is not the actual credential data generated by webauthn and is just dummy data so that the database is hit.
	// The next step is to modify our GORM credential model to fit with duo's functionality so that we only have to use one set of models.
	db, err := database.GetDB()
	if err != nil {
		jsonResponse(w, err.Error(), http.StatusInternalServerError)
	}
	gormCred := models.Credential{PublicKey: "publicKeyPlaceHolder"}
	err = db.AddCredentialToUser(username, gormCred)
	if err != nil {
		jsonResponse(w, err.Error(), http.StatusInternalServerError)
	}
	// End Work In Progress
